...
def _refresh_identity_confirmation(store, identity_id: Optional[str]) -> None:
    """Sync ExternalIdentity.confirmed to ≥1 confirmed IdentityLink edge."""
    if not identity_id:
        return
    ...
    for link in store.list_identity_links():
        ...
        if st != IdentityStatus.confirmed.value:
            continue
        if identity_id not in (link.left_identity_id, link.right_identity_id):
            continue
        still = True
        if link.entity_id and not entity_id:
...
def confirm_identity_link(
    ...
    *,
    ...
    # confirmed on ExternalIdentity means ≥1 confirmed edge — not "has Entity".
...
def unconfirm_identity_link(store, link_id: str) -> IdentityLink:
    """Roll a confirmed link back to candidate (manual undo).

    Rejected links are not reopened here — use an explicit reopen path later.
    """
    link = store.get_identity_link(link_id)
    if link is None:
        raise ValueError(f"identity link {link_id} not found")
    ...
    if st != IdentityStatus.confirmed.value:
        raise ValueError(
            f"identity link {link_id} is {st!r}; unconfirm requires confirmed"
        )

"""CLI command handlers used by argparse registration in ``twin.interfaces.cli``."""
...
def cmd_eval(args) -> None:
    from twin.interfaces import ux
    from twin.evals import compare_runs, default_eval_root, run_extraction_eval, run_retrieval_eval
...
def cmd_pack(args) -> None:
    from twin.interfaces import ux
    from twin.interfaces.commands import pack_cmd
    ...
    with ux.spinner("Building context pack…"):
        pack = pack_cmd.build_pack(ws, args)
    if args.json:
...
def cmd_privacy(args) -> None:
    from twin.privacy.deletion import preview_deletion
    from twin.privacy.engine import evaluate_access, explain_decision
    from twin.privacy.grants import create_grant, revoke_grant
    ...
    if cmd == "bootstrap":
        ...
        from twin.privacy.deletion import execute_deletion
        ...
        from twin.privacy.quarantine import release_quarantine
...
def cmd_cognize(args) -> None:
    ...
    if sub == "status":
        ux.print_ok(
            ...
            f"competing={data.get('competing_interpretations')} "
        ...
        if last:
            ux.print_dim(
                f"last_run={last.get('status')} halt={last.get('halt_reason') or '—'} "
                f"at={last.get('created_at')}"
            )
        return
    if sub == "review":
        ux.print_ok(
            f"open_reflections={data['counts']['open_reflections']} "
            f"competing_interpretations={data['counts']['competing_interpretations']}"
        )
        for r in data.get("open_reflections") or []:
            print(f"  [open] {r.get('text', '')[:120]}")
        for i in data.get("competing_interpretations") or []:
            print(f"  [intp] {i.get('explanation', '')[:120]}")
        return
...
def cmd_narrative(args) -> None:
    ...
    if sub == "show":
        ...
        data = cognize_cmd.narrative_commit_preview(ws, args)
    elif sub == "commit":

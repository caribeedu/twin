# Twin — redesign v2.6: longitudinal narratives, architecture vs pipeline
...
- donk8r follow-up (correlated evidence → information, not just independence) folded as **v2.4** — confidence ≠ agreement accumulation; silent reversal → low-confidence challenger + cross-source reinforcement; Sense coverage prerequisite; Event vs Situation left open.
...
- **v2.5** — identity vs GBrain; Architecture ≠ Cognize pipeline; **Interpretation** (ex-Proposal); **Narrative** (durable product); **Narrative Revision** (ex-Integrate-or-Separate); Understanding as *emergent*, not a table.
- **v2.6** — session wording; Narrative composition (not literary L1–L4 schema); stale **before** Cognize; confidence **read-time** + `same_originating_decision` + retain dissent (donk8r); open decisions settled (§10).
...
| Reddit: prefer “reflection/conclusion” over “memory” | Entity naming already on Reflection / Narrative |
...
**Memory** is demoted to everyday language or a *view* over committed Narratives — not the schema root. Prefer **reflection / interpretation / narrative** language over “memory.”
...
| Twin = another memory tool / GBrain clone | Prefer **Reflection / Interpretation / Narrative**; Understanding is emergent; memory is a view — see §0− |
...
2. Introduce schema types: Reflection, Interpretation, Relation (incl. `same_originating_decision`), **Narrative** (+ optional `grain`), **EpistemicState** without stored confidence scalar; migrate off Memory-as-root; no `understandings` table.  
...
*v2.6 — session = Sense+Inject edges (not a mode); Narrative composition via Relations/`grain`; confidence read-time + same_originating_decision + retain dissent; stale before Cognize; open decisions settled; Stage 6 surprise-first.*

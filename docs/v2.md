# Twin — redesign v2.7: longitudinal narratives, architecture vs pipeline, CLI command center
...
- **v2.7** — Twin CLI as **command center** (§12); cognition verbs retarget to v2 pipeline (`cognize` / `narrative` / `stance`; deprecate `extract`/`meditate`/`correlate` as primary UX).
...
*v2.7 — prior v2.6 cognition/epistemics; **§12 Twin CLI command center**; cognition CLI retargeted to v2 (`cognize` / `narrative` / `stance`; legacy extract/meditate/correlate as aliases).*

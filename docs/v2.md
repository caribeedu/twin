...
- **Code migration:** `twin memory …` and internal `Memory*` types are gone → `twin narrative …` / Relations / `TwinStore` / claims.  
...
| Today | Target |
|---|---|
| Interpret emits many similar review candidates | Raise few Reflections; many Interpretations compete under one question |
...
Yes — `extract`, `meditate`, and `correlate` are verbs of the **old** mental model (review candidates, brain-analogy episode chain). They must **not** be frozen into the Command Center as the long-term surface. The Center and the scripted CLI both retarget to Sense → Cognize → Inject.
...
| `twin extract` | Percept → review candidates | **`twin cognize`** (or `twin cognize run`) — full Cognize pipeline on pending percepts (**LLM-or-halt**) |
...
| `twin episode reflect` | Trajectory review candidates (Derived percepts + StoreClaims) | Raise Reflections / Form Interpretations inside Cognize; episode UX → **Situation** views |
| `twin review` | Confirm review candidates | Review **Interpretations** → **Commit Narrative**; always show Open Reflections |
...
During migration: retarget argv to v2 handlers (`twin meditate` → `twin cognize run`) so CI does not break overnight. Command Center **only shows v2 names**.

### 12.3 Information architecture (screens)

TUI Command Center (shipped) is a **thin supervisor** — Health · Services · MCP.
Browse / review / jobs / cognize / inject live in the Web Command Center
(`twin serve`) and scripted CLI.

```text
twin (Command Center TUI) — single screen
├── Health        doctor + checks (one section)
├── Services      serve / runtime — start · stop · logs
└── MCP (right)   client identity · preferred tools

twin serve (Web Command Center)
├── Home          dashboard sections
├── Explore       Narratives · Reflections · Interpretations · …
├── Review        Interpretations · commit Narrative
├── Cognize / Sense / Inject / Ops / Sessions
└── …
```

No bottom-bar menus. `q` quits center (offers to stop supervised children or
leave them daemonized — prefer **explicit** choice).
...
6. Cognize LLM-or-halt is the only meaning path; brain-stage CLI labels stay out of the product surface.  

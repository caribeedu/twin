...
| **v2.5** | Package walls & dual-read schema rename | Reorganize code to `sense` / `cognize` / `inject` / `store` / `llm` / `privacy` / `interfaces`; fold `cognition` into `cognize`; split `memory` + `judgment` by pipeline function; retarget MCP tool names; retire `MemoryItem` / `memory_id` dual-read names → store claim vocabulary; migrate DB; MCP `claim_*` only |

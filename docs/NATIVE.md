...
| Emit a context pack | On **SessionStart** (and explicit `pack_request`), and on the first **UserPromptSubmit** that **search-votes** a previously `unclassified` domain, Twin builds a firewall-filtered pack and prints it on stdout for the host to inject. Pack JSON extras include EpistemicState, open reflections, and derived confidence when Narratives exist. Stale Narratives are never injected as fresh. Conversation uses Sense+Inject edges — there is no session “mode.” Hook paths never wait on the local LLM for domain |

...
from twin.cognize.stance_engine.profile import load_profile, promote_claim, render_profile
...
def test_promote_preference_to_judgment(store, cfg, embedder):
    ...
    section = promote_claim(cfg.judgment_path, mem)
    assert section == "promoted_preferences"
    entries = load_profile(cfg.judgment_path)["promoted_preferences"]
    assert entries[0]["claim_id"] == mem.id
    # idempotent
    promote_claim(cfg.judgment_path, mem)

...
def test_backup_validate_and_sqlite_restore(store, tmp_path):
    ...
    mem = StoreClaim(
        id=ids.claim_id(), type=ClaimType.decision,
        title="SQLite backups", summary="Use file copy + NDJSON export",
        domain="technical", confidence=0.9, status=ClaimStatus.confirmed,
    )
    ...
    assert manifest.counts.get("claims", 0) >= 1
    ...
    try:
        assert other.get_claim(mem.id) is not None
...
def test_integrity_flags_confirmed_without_evidence(store):
    mem = StoreClaim(
        id=ids.claim_id(), type=ClaimType.fact,
